...
roster = pd.read_csv('2026-roster-info.csv')
players_basic = pd.read_csv('2026-players_basic.csv')
...
output_file = '2026-players_trank_merged.csv'
...
if len(unmatched_step1) > 0:
    ...
    unmatched_step1[['player_name', 'team']].to_csv('unmatched_trank_to_roster.csv', index=False)
...
if len(unmatched_step2) > 0:
    ...
    unmatched_step2[['player_name', 'team']].to_csv('unmatched_combined_to_basic.csv', index=False)
