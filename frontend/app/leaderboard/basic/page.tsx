...
import { ConferenceFilter } from "@/components/player/ConferenceFilter";
...
export default function BasicLeaderboard() {
  ...
  const [conferenceFilter, setConferenceFilter] = useState<"all" | "d1" | "high_major" | string>("all");
  ...
  // Map conference filter to API parameters
  const getConferenceParams = () => {
    if (conferenceFilter === "d1") {
      return { d1Only: true, highMajorOnly: undefined, conf: undefined };
    } else if (conferenceFilter === "high_major") {
      return { d1Only: undefined, highMajorOnly: true, conf: undefined };
    } else if (conferenceFilter === "all") {
      return { d1Only: undefined, highMajorOnly: undefined, conf: undefined };
    } else {
      // Individual conference
      return { d1Only: undefined, highMajorOnly: undefined, conf: conferenceFilter };
    }
  };

  const conferenceParams = getConferenceParams();

  // Fetch all players data (both basic and enriched) with current filters
  const { 
    players, 
    ...
    sort,
    order,
    year: year === "career" ? "career" : year,
    search: search || undefined,
    ...conferenceParams,
    // Remove dataTier filter to show all players
  });
  ...
  // Handle conference filter changes
  const handleConferenceChange = useCallback((newFilter: "all" | "d1" | "high_major" | string) => {
    setConferenceFilter(newFilter);
    setPage(0);
  }, []);
