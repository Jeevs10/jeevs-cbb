...
export default function PlayerStatsPanel({ player }) {
  ...
  const min_pg = perGame(player.Minutes, games);
  const pts_pg = perGame(player.Points, games);
  const ast_pg = perGame(player.Assists, games);
  const reb_pg = perGame(player["Rebounds Total"], games);
  const orb_pg = perGame(player["Rebounds Offensive"], games);
  const drb_pg = perGame(player["Rebounds Defensive"], games);
  const stl_pg = perGame(player.Steals, games);
  const blk_pg = perGame(player.Blocks, games);
  ...
  return (
    <div className="border-2 border-black bg-[#C7D0B8] text-black p-3 space-y-3 font-mono">
      ...
      <div className="flex flex-wrap gap-1 border-b border-black pb-2 text-black">
        ...
        <TabButton id="impact" label="IMPACT" />
        <TabButton id="role" label="ROLE" />
        <TabButton id="scoring" label="SCORING" />
        <TabButton id="defense" label="DEFENSE" />
        <TabButton id="rebounding" label="REBOUND" />
      </div>
      ...
      {tab === "role" && (
        <div className="space-y-2 text-black">
          <StatBar label="USAGE" value={player.Usage * 100} max={40} />
      ...
      {tab === "scoring" && (
        <div className="space-y-2 text-black">
          <StatBar label="eFG%" value={player.off_efg * 100} max={70} />
          <StatBar label="TS%" value={player.TrueShootingPct * 100} max={70} />
          <StatBar label="FTR" value={player.off_ftr * 100} max={60} />
          <StatBar label="FT%" value={player["FreeThrows Pct"] * 100} max={100} />
        </div>
      )}
      ...
      {tab === "defense" && (
        <div className="space-y-2 text-black">
          ...
          <StatBar label="DEF RTG" value={player.def_rtg} max={120} />
        </div>
      )}
